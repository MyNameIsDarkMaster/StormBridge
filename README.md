...
**SW:1-10** - микропереключатели выбора работы устройства. Положение активации - сдвинутое вниз, к плате.  

**Разъем P1** - служит для подключения SPI и других интерфейсов через преобразователь уровней.
...
Так же можно не подключать пин VCC, а выбрать напряжение обмена принудительно с помощью микропереключателей SW:1(1.8v), либо SW:2 (2.5v). При включенном положении данных переключателей пин VCC становится выходом и может питать подключенное устройство заданным напряжением (ток не более 100мА).  
...
**Разъем P2** - служит для подключения изолированного UART.  
...
Соответствие пинов разъема GPIO, пропускание согналов ограничено направленностью изолятора:
- TX - AD0
- RX - AD1  

**Разъем P3** - служит для подключения любых поддерживаемых интерфейсов. (UART, SPI, I2C, JTAG, GPIO).
...
:warning: **Осторожно** :exclamation: : пин AD7 подтянут к 3.3в через 51кОм. а так же служит катодом для светодиода "Flash".
