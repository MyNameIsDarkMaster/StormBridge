...
Если в системе уже есть *flashrom* другой версии, для удобной работы можно создать симлинк собранной версии программы, но с другим именем в директории ```/usr/bin```, например ```# ln -s flashrom /usr/bin/flashrom-dev```, запущенный в директории с собранной программой, создаст симлинк и позволит использовать команду ```flashrom-dev``` из любого каталога.
