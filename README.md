...
```
где;
-p ft2232_spi:type=232H,divisor=2,gpiol3=L - выбор программатора и настройка частоты обмена по формеле 60Мгц / divisor, divisor может быть только целым четным числом. Стабильная частота для конкретного чипа и шлейфа подбирается опытным путем, рекомендуемо для внутрисхемной прошивки выбирать divisor не меньше 6 (то есть 10Мгц и меньше). gpiol3=L - активация преобразователя логических уровней, через gpio AD7
